...
- **`fromVersion` is the version migrated FROM**, not to — and **who sets it depends on the form**. A `MigrationScript` is an `AutoInitDSLClass`: creating one through any MCP write tool (`mps_mcp_create_root_node`, `mps_mcp_insert_root_node_from_json`, `mps_mcp_update_node`) fires the concept's node factory, which sets `fromVersion` to the language's *current* version and bumps that version by 1 — nothing else to do. A **`PureMigrationScript` has no factory**: call `mps_mcp_update_module(moduleName = "<language>", operation = "SYNC_VERSION")` first, use the `languageVersion` it returns as the new script's `fromVersion`, insert the script, then call `SYNC_VERSION` again. The language version is never set by hand: `SYNC_VERSION` derives it from the scripts, is idempotent, and is safe to repeat. Do **not** hand-edit the `.mpl`: MPS holds the descriptor in memory and would overwrite the edit.
- **Any "language version" mismatch from the checker or the generator → `SYNC_VERSION`.** Its quick fix is unreachable (`check_root_node_problems(autoApplyQuickFixes = true)` will not repair it), but `SYNC_VERSION` does exactly what the IDE's "Correct Language Version" action does. Duplicate or missing versions are different: it reports them in `migrationProblems` without fixing them — correct the offending script's `fromVersion`, then sync again.
...
3. For a **`MigrationScript`**, creating it (step 4) sets `fromVersion` and bumps `languageVersion` via the concept's node factory — leave both alone. For a **`PureMigrationScript`**, which has no factory, call `mps_mcp_update_module(moduleName = "<language>", operation = "SYNC_VERSION")`, put the `languageVersion` it returns in the blueprint's `fromVersion`, insert the script (step 4), then call `SYNC_VERSION` again. If `migrationProblems` in its response is not empty, fix the listed scripts' `fromVersion` and sync again.
