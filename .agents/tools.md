...
- Never run `mps_mcp_initialize_project_for_agents` in this repository, including the delete-and-reinstall refresh its description and the `mps-mcp-workflow` skill recommend. The `mps-*` skills under `.agents/skills/` and `.claude/skills/` are hand-propagated copies of the blueprints in `plugins/mcp-tools/resources/jetbrains/mps/agents/mcp/skills/` — edit a blueprint and re-propagate by copying the changed skill folder over both catalogs (`SkillCatalogReplicationTest` fails when they diverge) instead of regenerating them, and never delete these copies to let the tool run.
