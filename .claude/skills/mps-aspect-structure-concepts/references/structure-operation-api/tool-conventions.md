...
The `mps_mcp_query_structure` operations that take one concept also accept the plural `conceptRefs`: a nonblank string or a nonempty array of nonblank strings. A single value or one-element array is read as `conceptRef`. With several values the first is used and a `warnings` entry names it, so query one concept per call. Any other shape is rejected, and so is `conceptRefs` sent beside `conceptRef` / `conceptReference`. `mps_mcp_alter_structure` rejects `conceptRefs`.
