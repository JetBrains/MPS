...
- **Blueprint files live in the system temp directory**: when passing a blueprint as a file path instead of inline JSON, the file must be inside the JVM system temp directory — `$TMPDIR` on macOS/Linux, `%TEMP%` on Windows; `/tmp` is also accepted on macOS. A leading `$TMPDIR` or `%TEMP%` is expanded. Example: `f="$TMPDIR/blueprint-$$.json"`.
