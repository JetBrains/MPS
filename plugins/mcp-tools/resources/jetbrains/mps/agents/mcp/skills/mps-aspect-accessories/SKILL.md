...
- **A language's OWN version is a different number**, exposed as `languageVersion` on every Language module in `mps_mcp_get_project_structure` and derived from the migration scripts by `mps_mcp_update_module(operation = "SYNC_VERSION")` (`max(fromVersion) + 1`, computed for you; there is no way to set it to an arbitrary value). It is the integer a migration script's `fromVersion` gates on, and both `MigrationScript` and `PureMigrationScript` get it from their node factory when created through a factory-running path — never set it by hand. Never hand-edit it in the `.mpl`, and never "fix" a migration by refreshing the consumer stamps instead — that marks the migration as already applied. See [module-info-fields.md](references/module-info-fields.md) and the `mps-aspect-migrations` skill.
