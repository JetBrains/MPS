...
- `mps_mcp_get_project_structure`: the universal tool to explore the project. Use `startingPoint` and filtering to avoid large responses; pass `includeStubModules=true` to include read-only libraries/stubs and modules from other open MPS projects. `data` is inline for small dumps and a temp-file path above `maxInlineBytes` (default 20000).
...
- `mps_mcp_create_module`, `mps_mcp_update_module` (operations: RENAME, CHANGE_VIRTUAL_FOLDER, DELETE). `mps_mcp_create_module` reports the models the new module already owns in `data.models` (`[{name, reference, aspect}]`) — a Language arrives with its aspect models and a Generator with `templates@generator`, so do not create them again.
...
- `mps_mcp_query_nodes`: read-only node queries — FIND_INSTANCES (find nodes of a concept; `sampleOnly` for one example), FIND_USAGES (nodes referencing a given node), GET_PARENT, GET_ROOT, GET_MODEL_FOR_NODE, NODE_INDEX, SIBLINGS, GET_CHILD_ROLE. Default scopes are selected-project based; explicit model/module/root scopes may target another open project read-only. List results are inline in `data` up to `maxInlineBytes` (default 20000), a temp-file path above it.
...
- `mps_mcp_print_node`: shows the underlying JSON structure or it shows the "visual" projection of a node. `format` accepts exactly three literals — `JSON` (default), `HTML`, `PLAIN TEXT` (with a **space**; `PLAIN_TEXT` and `text` are rejected). `nodeReference` must be a **node** reference; a model reference (`r:<uuid>(name)`) yields NOT_FOUND. `data` is the printout itself when it is at most `maxInlineBytes` (default 20000) and a temp-file path above that.
- `mps_mcp_insert_root_node_from_json`: bulk node creation; `json` accepts a single object **or a top-level JSON array** (inserted atomically). Leave the references empty if target nodes do not exist. Remember to set them later with `mps_mcp_update_node` (`SET`/`REFERENCE`).
- `mps_mcp_update_node`: unified node-mutation tool. One call covers add/set/delete on child, property, and reference roles. The node being mutated must belong to the selected project; reference targets may point to another open project if the target is in scope/imported. Selected via `operation` (`ADD`/`SET`/`DELETE`) × `kind` (`CHILD`/`PROPERTY`/`REFERENCE`). Valid combinations:
  ...
  - `childJson` accepts either an inline JSON string (max 4 KB) **or** an absolute path to a file containing the JSON blueprint — use the file form for large blueprints to avoid MCP-transport truncation. The file **must be inside the JVM system temp directory** (`$TMPDIR` on macOS/Linux — on macOS that is `/var/folders/...`, **not** `/tmp`; `%TEMP%` on Windows), otherwise the call fails with "Input file path … is not inside the system temp directory". One-liner: `f="$TMPDIR/blueprint-$$.json"`; if an agent file-writing tool (e.g. `Write`) creates the file, pass the expanded absolute path, since the tool does not expand `$TMPDIR`. The rejection message now names the accepted directory (`$TMPDIR` on macOS/Linux, `%TEMP%` on Windows).
  - `childJson` takes a **single JSON object only** — a top-level array is rejected (`Expected JsonObject but was JsonArray`). To add N children, call `ADD`/`CHILD` N times, or describe them all inside the parent's blueprint (that is what `mps_mcp_insert_root_node_from_json` accepts an array for).
  - `mps_mcp_insert_root_node_from_json` and `mps_mcp_update_node`: `responseDetail` = `summary` | `full`: `summary` is `{inserted|added, roots|nodes: [{name, reference, concept}], fixReferences}` and is the default from 10 top-level nodes up; `full` (one complete node envelope per created node) is the default below that and can always be forced.
- `mps_mcp_check_root_node_problems`: validation tool. Use this frequently to ensure your changes are correct. It accepts a node/root reference **or a model reference** (`r:<uuid>(name)`); the model form checks every root of the model in one call and is exhaustive — never re-check the roots individually after a clean model-level result. Each reported problem may carry a `quickFixes` array; `autoApplyQuickFixes=true` applies the fix of every problem carrying exactly one auto-applicable fix before returning the final report. The report is inline in `data` up to `maxInlineBytes` (default 20000), a temp-file path above it. On a model reference: `details.rootsChecked`, `details.scope`, per-root `roots` array; `perRoot=true` gives one compact entry per root.
...
- `mps_mcp_get_concept_details`: provides properties, children, and references for one concept/language or a JSON-array string in `conceptRefs` / `languageRefs`. Make/rebuild languages with `mps_mcp_alter_nodes` (`MAKE` with `rebuild=true`) if descriptors are hollow or stale. For the canonical structure-to-aspect editing and compilation prerequisite chain, see the Critical Directives in the `mps-mcp-workflow` skill. `detail="shape"` returns only the structural projection (no docs, no `sampleNode`), `includeChildRoleConcepts=true` adds the one-hop child/reference role targets as `relatedConcepts`, and `data` is inline up to `maxInlineBytes` (default 20000). Name suggestions for an unresolved ref never point into another open project.
