...
- `mps_mcp_print_node`: shows the underlying JSON structure or it shows the "visual" projection of a node. `format` accepts exactly three literals — `JSON` (default), `HTML`, `PLAIN TEXT` (with a **space**; `PLAIN_TEXT` and `text` are rejected). `nodeReference` must be a **node** reference; a model reference (`r:<uuid>(name)`) yields NOT_FOUND.
...
- `mps_mcp_check_root_node_problems`: validation tool. Use this frequently to ensure your changes are correct. It accepts a node/root reference **or a model reference** (`r:<uuid>(name)`); the model form checks every root of the model in one call and is exhaustive — never re-check the roots individually after a clean model-level result. Each reported problem may carry a `quickFixes` array; `autoApplyQuickFixes=true` applies the fix of every problem carrying exactly one auto-applicable fix before returning the final report.
